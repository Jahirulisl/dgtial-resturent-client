...
const UpdateItem = () => {
  const { name, category, recipe, price, image: oldImage, _id: _id } = useLoaderData();
  //for new image state
    ...
    if (res.data.success) {
      ...
      if (menuRes.data.modifiedCount > 0) {
        ...
        Swal.fire({
          position: "top-end",
          icon: "success",
          title: `${data.name} is updated to the menue`,
      ...
      <div>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className='form-control'>
            ...
            <textarea defaultValue={recipe} {...register('recipe')} className="textarea textarea-border w-full h-24 " placeholder="Bio"></textarea>
          </div>

          {/*for file/ image*/}
          <div>
            <input {...register('image', { required: true })} type="file" className="file-input w-full my-6" onChange={e => setNewImage(e.target.files[0])} />
